...
bool CSHA1::HashFile(const TCHAR* tszFileName)
{
	...
	#else
		FILE* fpIn = fopen(TCHAR_TO_UTF8(tszFileName), "rb");
	#endif
...
bool CSHA1::ReportHash(TCHAR* tszReport, REPORT_TYPE rtReportType) const
{
	...
	if((rtReportType == REPORT_HEX) || (rtReportType == REPORT_HEX_SHORT))
	{
		...
		#else
			snprintf(TCHAR_TO_UTF8(tszTemp), 15, "%02X", m_digest[0]);
		#endif
		#if PLATFORM_WINDOWS
			_tcscpy(tszReport, tszTemp);
		#else  
			strcpy(TCHAR_TO_UTF8(tszReport), TCHAR_TO_UTF8(tszTemp));
		#endif

		FString FmtString = ((rtReportType == REPORT_HEX) ? TEXT(" %02X") : TEXT("%02X"));
		const TCHAR* lpFmt = *FmtString;
		//const TCHAR* lpFmt = ((rtReportType == REPORT_HEX) ? (const TCHAR*)TEXT(" %02X") : (const TCHAR*)TEXT("%02X"));
		for(size_t i = 1; i < 20; ++i)
		{
		#if PLATFORM_WINDOWS
			_sntprintf(tszTemp, 15, lpFmt, m_digest[i]);
		#else
			snprintf(TCHAR_TO_UTF8(tszTemp), 15, TCHAR_TO_UTF8(lpFmt), m_digest[i]);
		#endif
		#if PLATFORM_WINDOWS
			_tcscpy(tszReport, tszTemp);
		#else  
	...
	#else
		snprintf(TCHAR_TO_UTF8(tszTemp), 15, "%u", m_digest[0]);
		strcpy(TCHAR_TO_UTF8(tszReport), TCHAR_TO_UTF8(tszTemp));

		for(size_t i = 1; i < 20; ++i)
		{
			snprintf(TCHAR_TO_UTF8(tszTemp), 15, " %u", m_digest[i]);
			strcat(TCHAR_TO_UTF8(tszReport), TCHAR_TO_UTF8(tszTemp));
		}
	#endif
	}
	else return false;
