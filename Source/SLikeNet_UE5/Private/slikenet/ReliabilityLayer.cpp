...
{
    if (datagramHistory.IsEmpty()) return;
    
    DatagramSequenceNumberType offsetIntoList = index - datagramHistoryPopCount;
   
    if (offsetIntoList >= datagramHistory.Size()) return;
    
    MessageNumberNode *mnm = datagramHistory[offsetIntoList].head;
    if (mnm == (MessageNumberNode*)-1 || mnm == nullptr) return;
    MessageNumberNode *next;
    while (mnm && mnm != (MessageNumberNode*)-1)
    {
        next = mnm->next;
